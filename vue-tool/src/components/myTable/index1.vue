<template>
  <div class="my-table-container">
    <div class="selection-box" v-if="tableData.showSelection && selectionList.length">
      <div class="text">
        <i class="icon-Shape icon-style"></i>已选择
        <span>{{ selectionList.length }}</span>项, 您可以对其批量删除等操作
      </div>
    ...
    <el-table
      ...
      :header-cell-style="{ background:'#fafafa', color:'#606266', textAlign: 'left',fontSize: '14px' }"
      ...
      <template v-for="item in tableData.arr">
        ...
        >
          <template slot-scope="scope">
            <div class="icon-status-box" v-if="item.type == 'icon-status' && scope.row[item.prop]">
              <img
                v-if="Number(scope.row[item.prop])"
                :src="item.statusMap[Number(scope.row[item.prop])] && item.statusMap[Number(scope.row[item.prop])].icon || ''"
              />
              <span>{{item.statusMap[Number(scope.row[item.prop])] && item.statusMap[Number(scope.row[item.prop])].label || ''}}</span>
            </div>
            <div class="status-box" v-else-if="item.type == 'status' && scope.row[item.prop]">
              <span
                :style="item.oStyle ? {...item.oStyle(scope.row)} : {}"
              >{{item.statusMap[Number(scope.row[item.prop])] && item.statusMap[Number(scope.row[item.prop])].label || ''}}</span>
            </div>
            <div class="status-box" v-else-if="item.type == 'unit' && scope.row[item.prop]">
              <span>{{`${scope.row[item.prop]} ${scope.row[item.mergeField]}`}}</span>
            </div>
            <div class="icon-sign-box" v-else-if="item.type == 'icon-sign' && scope.row[item.prop]">
        ...
        >
          <template slot-scope="scope">
            {{ item.text ? item.text(scope.row) : ( scope.row[item.prop] ? scope.row[item.prop] : (scope.row[item.prop] === 0 ? 0 : '-'))}}
          </template>
        </el-table-column>
        <!-- 序号 -->
        <el-table-column
        ...
        >
          ...
          <template slot-scope="scope">
            ...
            <div
              :style="item.oStyle ? {...item.oStyle(scope.row)} : {}"
              v-else
            >{{ item.text ? item.text(scope.row) : ( scope.row[item.prop] ? scope.row[item.prop] : (scope.row[item.prop] === 0 ? 0 : '-'))}}</div>
          </template>
        </el-table-column>
      </template>
    </el-table>
